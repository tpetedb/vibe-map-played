...
# new tab:
#
#   source /path/to/vibe-map/scripts/vibe.zsh
#
...
export VIBE_HOME="${0:A:h:h}"

alias g='uv run --project "$VIBE_HOME" --no-sync vibe'
alias gs='g status'
alias gc='g check'
...
evening() {
  cd "$VIBE_HOME" || return 1
  just game
  open -a Obsidian "$VIBE_HOME/vault" 2>/dev/null
  claude
}
...
vibe_prompt_segment() {
  local json
  json=$(uv run --project "$VIBE_HOME" --no-sync vibe status --json 2>/dev/null) || return 0
