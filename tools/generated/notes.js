...
**Try in five minutes.** In Claude Code type \`/help\`, then \`!uv run vibe status\`, then \`@vibe.toml what does the pet table do\`. Open \`CLAUDE.md\` and see the \`@AGENTS.md\` import at the top. Then open the vault in Obsidian and click a \`#tech\` tag.
...
**Languages and code.** Python first, then the web, the other languages and how to know the code works. [[Python]], [[Python libraries: what they are for]], [[HTML, CSS and JavaScript]], [[Other languages and what they are for]], [[Tests and evals]]
