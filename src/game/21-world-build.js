...
function buildWorld(id){
  ...
  plots.length=0;clouds.length=0;parts.length=0;for(const k in builds)delete builds[k];for(const k in props)delete props[k];obstacles=[];
  PLOT_POS=W.plots.map(p=>new T.Vector3(p[0],0,p[1]));
  scene=new T.Scene();scene.fog=new T.Fog(W.fog,60,150);
  hemiL=new T.HemisphereLight("#cfe9ff","#4a7a3a",.7);scene.add(hemiL);ambL=new T.AmbientLight("#fff",.15);scene.add(ambL);
  dirL=new T.DirectionalLight("#fff5d6",1.1);dirL.position.set(14,24,10);dirL.castShadow=true;dirL.shadow.mapSize.set(CONFIG.shadowMap,CONFIG.shadowMap);Object.assign(dirL.shadow.camera,{left:-28,right:28,top:28,bottom:-28,near:1,far:80});scene.add(dirL);
  const ex=k=>W.extras.includes(k);const dummy=new T.Object3D();
  // landmass
  const land=new T.Group();const blob=(x,z,r,seg)=>{const g=cyl(r,r+1.2,2.4,W.grass,x,-1.2,z,seg||9);const d=cyl(r+1.1,r+2.2,2.2,W.dirt,x,-2.6,z,seg||9);land.add(g,d);return g};
  island=blob(...W.land[0],12);W.land.slice(1).forEach(b=>blob(...b));scene.add(land);island.userData.parts=land.children.filter((m,i)=>i%2===0);
  // sea
  wGeo=new T.PlaneGeometry(220,220,56,56);wGeo.rotateX(-Math.PI/2);wBase=wGeo.attributes.position.array.slice();
  ...
  const inst=new T.InstancedMesh(new T.BoxGeometry(1.15,.07,.42),mat(id==="prod"?"#8A8A98":id==="winter"?"#C9D6E2":id==="desert"?"#C9A24E":"#E9D9B5"),N);
  ...
  if(ex("stall")){const stall=new T.Group();stall.add(box(2,.9,1,"#A9743F",0,.45,0));stall.add(box(2.4,.1,1.4,"#E11D48",0,2,0));[-1,1].forEach(x=>stall.add(cyl(.05,.05,2,"#7B5128",x,1,.5,4)));[["#FFBF00",-.6],["#22C55E",0],["#F87171",.6]].forEach(([c,x])=>stall.add(sph(.22,c,x,1.05,.2,6)));stall.position.set(4.5,0,2);stall.rotation.y=-.6;scene.add(stall);obstacles.push([4.5,2,1.3])}
  if(ex("well")){const well=new T.Group();well.add(cyl(.7,.75,.8,"#9CA3AF",0,.4,0,8));well.add(cone(.9,.6,"#7B5128",0,1.9,0,6));[-.5,.5].forEach(x=>well.add(box(.1,1.4,.1,"#7B5128",x,1,0)));well.position.set(-3,0,-4);scene.add(well);obstacles.push([-3,-4,.9])}
  if(ex("mountain")){const mt=new T.Group();mt.add(cone(6,7,id==="winter"?"#B8C4CF":"#8B7355",0,3.5,0,7));mt.add(cone(2.3,2.6,"#F8FAFC",0,5.7,0,7));mt.position.set(-9,0,-14);mt.traverse(o=>{if(o.isMesh)o.castShadow=true});scene.add(mt);obstacles.push([-9,-14,4.5])}
  if(ex("volcano")){const v=new T.Group();v.add(cyl(2,6.5,7,"#2E2E38",0,3.5,0,8));v.add(cyl(1.4,2.1,.5,"#FF4500",0,7.1,0,8).material.emissive.set("#FF4500")?cyl(1.4,2.1,.5,"#FF6A1A",0,7.1,0,8):null);const glow=new T.PointLight("#FF4500",2.5,18);glow.position.y=8;v.add(glow);const sm=[];for(let i=0;i<8;i++){const m=new T.Mesh(new T.SphereGeometry(.7,6,6),new T.MeshStandardMaterial({color:"#4B4B55",transparent:true,opacity:.6}));m.userData.o=i;v.add(m);sm.push(m)}v.userData.smoke=sm;v.position.set(-9,0,-14);scene.add(v);props.volcano=v;obstacles.push([-9,-14,5.5]);
    for(let i=0;i<4;i++){const l=new T.Mesh(new T.BoxGeometry(1.5,.15,4+i),wmat);l.position.set(-9+i*1.5,.06,-9.5+i*.5);l.rotation.y=.4-i*.25;scene.add(l)}}
  if(ex("harbour")){const dock=new T.Group();for(let i=0;i<9;i++)dock.add(box(.5,.12,2,"#D9A76A",i*.55,.1,0));[0,2.2,4.4].forEach(x=>{dock.add(cyl(.1,.12,1.4,"#7B5128",x,-.4,-1));dock.add(cyl(.1,.12,1.4,"#7B5128",x,-.4,1))});const hp=id==="winter"?[15,-5]:[15,-8];dock.position.set(hp[0],0,hp[1]);scene.add(dock);
  ...
  if(ex("airstrip")){const p1=plane("#FF8C1A");p1.position.set(1.5,0,-16.5);scene.add(p1);props.plane=p1}
  const p2=plane(id==="prod"?"#7F1D1D":"#EF4444");scene.add(p2);props.plane2=p2;
  if(ex("balloon")){const bal=new T.Group();bal.add(sph(1.8,"#FF8C1A",0,3.4,0,10));const st2=sph(1.82,"#FFBF00",0,3.4,0,10);st2.scale.set(.5,1,1);bal.add(st2);bal.add(box(.9,.7,.9,"#A9743F",0,0,0));[-.4,.4].forEach(x=>bal.add(cyl(.03,.03,2.3,"#1F2937",x,1.2,0,3)));bal.position.set(-15,7,11);scene.add(bal);props.balloon=bal}
  ...
  const treeSpots=[[5,-7,0],[-4,-8,1],[9,-13,0],[-13,-8,0],[-15,11,1],[-11,12,0],[3,15,1],[12,9,0],[17,-1,1],[8,3,1],[-6,-2,0],[6,-3,1],[-1,-13,0],[15,-13,0],[-12,-14,0]];
  treeSpots.forEach(a=>{if(onLandW(a[0],a[1])&&!PLOT_POS.some(p=>p.distanceTo(new T.Vector3(a[0],0,a[1]))<2.8))treeAt(...a)});
  [[7,7],[-8,-6],[2,-6],[-14,4],[10,-2]].forEach(([x,z])=>{if(onLandW(x,z))scene.add(sph(.45,id==="prod"?"#1F1F27":"#9CA3AF",x,.2,z,5))});
  if(ex("flowers")){const fl=new T.InstancedMesh(new T.BoxGeometry(.18,.18,.18),new T.MeshStandardMaterial({color:"#fff",flatShading:true}),160);const colr=new T.Color();for(let i=0;i<160;i++){const a=Math.random()*Math.PI*2,r=3+Math.random()*11;dummy.position.set(Math.cos(a)*r,.12,Math.sin(a)*r);dummy.rotation.set(0,Math.random(),0);dummy.updateMatrix();fl.setMatrixAt(i,dummy.matrix);colr.set(["#FF8C1A","#FFBF00","#F8FAFC","#FFA94D"][i%4]);fl.setColorAt(i,colr)}scene.add(fl)}
  if(ex("cacti")){[[6,-6],[-5,-7],[10,5],[-9,1],[3,-15],[14,-4]].forEach(([x,z])=>{if(!onLandW(x,z))return;const c=new T.Group();c.add(cyl(.3,.35,2,"#2F855A",0,1,0,7));c.add(cyl(.18,.2,.9,"#2F855A",.5,1.5,0,6));c.add(cyl(.18,.2,.7,"#2F855A",-.5,1.2,0,6));c.position.set(x,0,z);scene.add(c);obstacles.push([x,z,.6])})}
  if(ex("mesas")){[[-8,-14,4,3.5],[12,-13,3,2.5],[-16,8,2.5,2]].forEach(([x,z,r,h])=>{const m=new T.Group();m.add(cyl(r*.8,r,h,"#B45309",0,h/2,0,8));m.add(cyl(r*.75,r*.8,.4,"#D97706",0,h+.2,0,8));m.position.set(x,0,z);m.traverse(o=>{if(o.isMesh)o.castShadow=true});scene.add(m);obstacles.push([x,z,r])})}
  if(ex("dunes")){[[6,8],[-6,9],[8,-9],[-11,-4]].forEach(([x,z])=>{const d=sph(3,"#F1DDA2",x,-2.2,z,10);d.scale.set(1,.5,1);scene.add(d)})}
  ...
  if(ex("snowmen")){[[6,-4],[-7,9]].forEach(([x,z])=>{const g=new T.Group();g.add(sph(.7,"#F8FAFC",0,.6,0,9));g.add(sph(.5,"#F8FAFC",0,1.55,0,9));g.add(sph(.36,"#F8FAFC",0,2.25,0,9));g.add(cone(.1,.4,"#F97316",0,2.25,.5,5).rotateX(Math.PI/2)?cone(.1,.4,"#F97316",0,2.25,.5,5):null);g.add(box(.6,.1,.6,"#1F2937",0,2.55,0));g.add(box(.4,.35,.4,"#1F2937",0,2.75,0));g.position.set(x,0,z);scene.add(g);obstacles.push([x,z,.9])})}
  if(ex("icefloes")){for(let i=0;i<10;i++){const a=i/10*Math.PI*2,r=24+(i%3)*3;const f=cyl(1+(i%3)*.5,1.2+(i%3)*.5,.3,"#E8F3FA",Math.cos(a)*r,-1.5,Math.sin(a)*r,6);scene.add(f)}}
  if(ex("snow")){const g=new T.BufferGeometry();const v=[];for(let i=0;i<500;i++)v.push((Math.random()-.5)*60,Math.random()*20,(Math.random()-.5)*60);g.setAttribute("position",new T.Float32BufferAttribute(v,3));const pts=new T.Points(g,new T.PointsMaterial({color:"#fff",size:.25,transparent:true,opacity:.9}));scene.add(pts);props.snow=pts}
  ...
  for(let i=0;i<8;i++){const c=new T.Group();[[0,0,0,1.6],[1.5,.3,.2,1.1],[-1.4,.2,.3,1],[.4,.7,-.3,.9]].forEach(([x,y,z,r])=>{const m=new T.Mesh(new T.SphereGeometry(r,7,7),new T.MeshStandardMaterial({color:id==="prod"?"#6B6B78":"#fff",flatShading:true,transparent:true,opacity:.92}));m.position.set(x,y,z);c.add(m)});c.position.set(-40+i*11,13+Math.sin(i)*2.5,-22+((i*7)%14));c.userData.v=.4+i*.08;scene.add(c);clouds.push(c)}
  ...
  const sg=new T.BufferGeometry();const sv=[];for(let i=0;i<400;i++){const a=Math.random()*Math.PI*2,b=Math.random()*Math.PI*.5;sv.push(Math.cos(a)*Math.cos(b)*120,Math.sin(b)*120+5,Math.sin(a)*Math.cos(b)*120)}sg.setAttribute("position",new T.Float32BufferAttribute(sv,3));stars=new T.Points(sg,new T.PointsMaterial({color:"#fff",size:.6,transparent:true,opacity:0}));scene.add(stars);
