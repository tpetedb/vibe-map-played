window.start=function(){S.name=$("name").value.trim()||"Lotte";save();$("title").classList.add("off");if(!started){try{if(typeof THREE==="undefined")throw new Error("three.js not loaded");init3d();started=true}catch(e){__err("3D failed: "+(e&&e.message||e)+". Falling back to the Roadmap list.");openSheet("s-map")}}hud();say(S.done.length===8?"fin":"walk")};
...
window.enterNear=function(){if(typeof nearK==="string"&&nearK.startsWith("m:"))openMentor(nearK.slice(2));else if(nearK)open(nearK)};
window.openCh=open;
...
function open(n){
  ...
  if(n===1&&!S.creature)summon();
  if(n===2)renderCreature("creature2");if(n===3)drawChart();if(n===4)renderVersions();if(n===5)renderBridges();if(n===9)renderFinale();
  say(n);openSheet("s-"+n);
}
function renderMap(){const ok=S.done.length===8;const ev=CAMPAIGN[S.world];
  ...
  $("plotlist").innerHTML=`<div class="evening">${ev.title}</div><p class="small muted">${ev.blurb}</p>`+preflight+CH.map((c,i)=>{const k=i+1,done=S.done.includes(k),locked=k>1&&!S.done.includes(k-1);
    return `<button class="date${done?' pick':''}" ${locked?'disabled':''} onclick="openCh(${k})">${c.h}, ${c.n}${done?" (delivered)":locked?" (blocked by dependency)":""}<br><span class="small" style="opacity:.75">${c.d}</span></button>`}).join("")+
    (S.world==="campus"?`<button class="date" ${ok?'':'disabled'} onclick="openCh(9)">Calendar alignment${ok?"":" (pending eight OKRs)"}</button>`:"")+
    `<div class="card"><h3>Your path: the mentors</h3><p class="small muted">People you met on the islands, and what you chose. Tap to revisit or change your mind.</p>`+MENTORS.map(m=>{const st=S.path[m.id];return `<div class="pathrow"><span><b>${m.name}</b> <span class="muted">· ${WORLDS[m.world].name}</span></span><span style="display:flex;gap:6px;align-items:center"><span class="st ${st||''}">${st==="deep"?"on path":st==="skip"?"skipped":"not met"}</span><button onclick="openMentor('${m.id}')" style="padding:4px 10px;font-size:12px">Open</button></span></div>`}).join("")+`</div>`+
    `<div class="card"><h3>The campaign</h3>`+Object.keys(CAMPAIGN).map(k=>`<div class="pathrow"><span><b>${CAMPAIGN[k].title}</b><br><span class="muted small">${WORLDS[k].name}</span></span><span style="display:flex;gap:6px;align-items:center"><span class="st ${(S.doneW[k]||[]).length===8?'deep':''}">${(S.doneW[k]||[]).length}/8</span><button onclick="setWorld('${k}');closeSheet()" style="padding:4px 10px;font-size:12px">Go</button></span></div>`).join("")+`</div>`;
}
