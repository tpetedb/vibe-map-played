...
function label(text,scale=1){const cv=document.createElement("canvas");cv.width=256;cv.height=64;const g=cv.getContext("2d");g.font="bold 30px Inter,sans-serif";g.textAlign="center";g.fillStyle="rgba(0,0,0,.45)";g.beginPath();const w=g.measureText(text).width+28;g.roundRect?g.roundRect(128-w/2,10,w,44,22):g.rect(128-w/2,10,w,44);g.fill();g.fillStyle="#fff";g.fillText(text,128,42);
  const sp=new T.Sprite(new T.SpriteMaterial({map:new T.CanvasTexture(cv),transparent:true,depthTest:false}));sp.scale.set(4*scale,1*scale,1);return sp}
