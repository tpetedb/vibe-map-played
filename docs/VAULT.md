...
`vault/` is an Obsidian vault: a folder of Markdown files that the Grimoire CLI writes and you read, link and extend. Open it in Obsidian for the graph; open it in any editor for the text. Nothing in it needs a community plugin.

## Layout

| Path | What lives there |
|---|---|
| `vault/Grimoire/` | every note, flat, one file per topic |
| `vault/Grimoire/_templates/` | Obsidian templates for the four note kinds, hidden from the graph |
...
- **One note per finished workstream**, written by `grimoire done`.
...
In Obsidian, click the **Vault profile** icon at the bottom left, choose **Manage vaults...**, click **Open** next to *Open folder as vault* and pick `vault/`. The committed `.obsidian/` folder brings the dark base, the `grimoire` CSS snippet (if it shows as off under Settings, Appearance, CSS snippets, click reload and toggle it on), the graph colour groups, and the Templates core plugin pointed at `Grimoire/_templates`. In a new note, run **Templates: Insert template** from the command palette and pick Workstream, Concept, Person or Decision; `{{title}}`, `{{date}}` and `{{time}}` fill themselves in.
...
| `wiki-ingest`, `save`, transaction bundles | `grimoire done`, `grimoire vault build`, or an agent using the `obsidian-notes` skill |
| `wiki-lint` | `grimoire vault lint` |

What to run:

- `just vault`, or `uv run grimoire vault build` then `uv run grimoire vault lint`, after every session.
...
- Do not run `/claude-obsidian:wiki` (init or adopt) against `vault/`. It would scaffold `wiki/`, `.raw/` and ledgers next to `Grimoire/`, and its lint only reads `wiki/`. The CLI owns this vault's structure.

## Lint checklist

`grimoire vault lint` reports the first two; check the rest by eye.
...
7. Map: `Map.md` renders after `grimoire map`.
