---
name: install-camp
description: Installs, sets up and runs Vibe Code Camp (Vibe Code Camp) on this Mac, with the user choosing every dependency. Use for "set up vibe", "install the course", "get me ready for the evening", "is my machine ready", "what do I need installed", "start the evening", or to run the game, the CLI, the tests or the vault.
---
# Install and use Vibe Code Camp

Nothing is installed without the user choosing it. Present the menu first, then act only on the chosen items. `uv run vibe start` (or `just start`) is the interactive version of this menu; offer it when uv is already there.
...
- Run the game: `just game` or `open game/vibe-map.html`
- Progress: `uv run vibe status`; sync with the game via `export` and `import` (the camp-progress skill)
- Vault: open `vault/` in Obsidian; `uv run vibe vault build` rebuilds the notes and the map
