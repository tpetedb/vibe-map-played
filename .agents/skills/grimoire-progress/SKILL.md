...
description: Tracks the eight Grimoire workstreams, awards XP and records what was built, through the grimoire CLI. Use when the user finished a workstream ("mark 3 done", "I built the chart"), asks "where am I", "what is next", "check my progress", wants the progress map or Map.md, or wants to export or import the progress code that syncs with the game.
allowed-tools: Bash(uv run grimoire *) Bash(just *)
---
# Grimoire progress

The CLI is the source of truth: `uv run grimoire <command>` (`uv run grimoire --help` lists them; `just status`, `just check` and `just vault` wrap the common ones).
