---
name: python-data
description: Writes small, readable Python for reading data/scores.csv, computing summaries and drawing a chart, run with uv. Use when asked for a script, a chart or plot, "automate this", "do it in Python", polars or pandas, or when SQL is not the right tool.
---
# Python for data
...
- Read `data/scores.csv` with `csv.DictReader` (or `polars.read_csv`). Never mutate it.
...
Starting point: `python/scores.py` (`uv run python python/scores.py`).
