---
name: semver
description: Picks the next version number with Semantic Versioning 2.0.0 (MAJOR.MINOR.PATCH) and bumps it in pyproject.toml, vibemap/__init__.py and a git tag. Use when the user says "bump the version", "what version is this", "is this a breaking change", "cut a release", "release 0.3.0", "tag it", or asks what major, minor, patch, 0.x or a pre-release like 1.0.0-rc.1 means.
...
- The number lives in two places here, `pyproject.toml` and `vibemap/__init__.py`, and they must agree. Tag the commit `v0.3.0` so git and `CHANGELOG.md` say the same thing.
...
2. Read CHANGELOG.md, section [Unreleased]:
     ...
     Added                                         -> MINOR
     only Fixed                                    -> PATCH
3. Set the number in pyproject.toml and vibemap/__init__.py
4. Rename [Unreleased] to [0.3.0] - YYYY-MM-DD and fix the links at the bottom
5. uv run vibe --version prints 0.3.0; commit "Release 0.3.0: <why>"
