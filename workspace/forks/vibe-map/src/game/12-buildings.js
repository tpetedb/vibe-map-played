function building(k){
  ...
  if(k===1){for(let i=0;i<6;i++){const a=i/6*Math.PI*2;g.add(box(.5,1+((i*7)%3)*.3,.5,"#B3A08B",Math.cos(a)*1.6,.5,Math.sin(a)*1.6))}
    const cr=new T.Mesh(new T.OctahedronGeometry(.7),mat("#FFA94D",{emissive:"#6D28D9",emissiveIntensity:.6}));cr.position.y=1.6;g.add(cr);g.userData.spin=cr;
  ...
  if(k===2){g.add(box(2.2,1.6,2,"#E7C9A0",0,.8,0));g.add(cone(1.8,1.2,"#B44A46",0,2.2,0,4));g.children[1].rotation.y=Math.PI/4;
    g.add(box(.5,.8,.1,"#5A3C22",.5,.4,1.02));g.add(box(.5,.5,.1,"#7DB8E6",-.5,.9,1.02));g.add(box(.9,.7,.1,"#FFF3C2",-1.5,.9,.6));g.add(box(.08,.9,.08,"#7B5128",-1.5,.45,.6))}
  if(k===3){g.add(box(2.8,1.8,2.2,"#F1D8A5",0,.9,0));g.add(cone(2.2,1.3,"#7B9C4B",0,2.45,0,4));g.children[1].rotation.y=Math.PI/4;
    [.5,.9,.7,1.2].forEach((h,i)=>g.add(box(.25,h,.1,"#3B2F2A",-.6+i*.4,h/2+.1,1.12)));g.add(box(.5,.5,.1,"#7DB8E6",1,1.1,1.12))}
  if(k===4){g.add(cyl(.9,1.1,4,"#DCD3C4",0,2,0,8));g.add(cone(1.3,1.4,"#4A5D8A",0,4.6,0,8));g.add(cyl(.05,.05,2,"#5A3C22",0,6.2,0,4));
    const flag=new T.Mesh(new T.PlaneGeometry(1.2,.7,6,1),new T.MeshStandardMaterial({color:"#F04923",side:T.DoubleSide,flatShading:true}));flag.position.set(.62,6.8,0);g.add(flag);g.userData.flag=flag;
    [1.2,2.2,3.2].forEach(y=>g.add(box(.3,.4,.1,"#222",0,y,1.0)))}
  ...
  if(k===7){g.add(box(2.2,.5,2.2,"#9CA3AF",0,.25,0));for(let i=0;i<4;i++){const a=i*Math.PI/2;g.add(cyl(.08,.1,6,"#CBD5E1",Math.cos(a)*.7,3,Math.sin(a)*.7,4))}
    ...
    const bb=box(2.6,1.2,.12,"#0EA5E9",0,4.2,1.1);g.add(bb);const lab=label("LIVE",.7);lab.position.set(0,4.2,1.3);g.add(lab)}
  if(k===8){g.add(box(3,1.8,2.4,"#64748B",0,.9,0));g.add(box(3.2,.3,2.6,"#475569",0,1.95,0));g.add(cyl(.3,.35,1.6,"#94A3B8",1,2.8,-.6,6));
    const gear=new T.Mesh(new T.TorusGeometry(.8,.22,6,10),mat("#FFBF00"));gear.position.set(-.6,2.9,0);g.add(gear);g.userData.gear=gear;
    for(let i=0;i<8;i++){const a=i/8*Math.PI*2;const t=box(.3,.3,.3,"#FFBF00",-.6+Math.cos(a)*1.05,2.9+Math.sin(a)*1.05,0);t.rotation.z=a;g.add(t)}
    g.add(box(.6,.9,.1,"#1F2937",0,.45,1.22));const sm=[];for(let i=0;i<5;i++){const m=new T.Mesh(new T.SphereGeometry(.25,6,6),new T.MeshStandardMaterial({color:"#E5E7EB",transparent:true,opacity:.6}));m.position.set(1,3.6+i*.6,-.6);m.userData.o=i;g.add(m);sm.push(m)}g.userData.smoke=sm}
