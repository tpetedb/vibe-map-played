...
`grimoire check 3` runs the checks for workstream 3 of the current world and
...
from grimoire.vault import safe_title
...
def _vault_report(cfg: Config):
    from grimoire.vault import Vault  # local: vault imports quests
...
def _c3_strict(cfg: Config) -> tuple[bool, str]:
    from grimoire.scores import run_sql
...
def _note_check(world: str, n: int) -> Check:
    ...
    return Check(
        ...
        fn,
        f"Write vault/Grimoire/{ws.name}.md: what you learned, two [[links]], "
        "a dated section.",
    )
...
CAMPUS_CHECKS: dict[int, tuple[Check, ...]] = {
    ...
    6: (
        Check(
            "a linked vault",
            _c6_vault,
            "Six notes and twelve wikilinks; run grimoire vault build.",
        ),
        Check("no dead links", _c6_strict, "grimoire vault lint lists them.", "strict"),
    ),
    7: (
