...
in themes/ with the same fields; `grimoire theme create` asks the provider to
write one.
"""
...
class Theme:
    ...
    def to_toml(self) -> str:
        d = asdict(self)
        lines = [
            f"# Theme {self.id}: edit freely, then set [theme] preset in grimoire.toml"
        ]
        for k, v in d.items():
