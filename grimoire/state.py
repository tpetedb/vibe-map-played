...
.grimoire/state.json is written by the CLI and the TUI; the game keeps its own
...
STATE_PATH = ROOT / ".grimoire" / "state.json"
...
class State(BaseModel):
    ...
    def load(cls, path: Path = STATE_PATH) -> State:
        ...
        if version > STATE_VERSION:
            raise ValueError(
                f"{path} is version {version}; this grimoire reads up to "
